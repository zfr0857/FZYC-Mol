...
def minority_operating_point(
    y_train: np.ndarray,
    y_validation: np.ndarray,
    pred: np.ndarray,
    target_recall: float = 0.80,
) -> dict[str, float]:
    """Learn a validation threshold under a training-defined minority-class recall constraint."""
    counts = np.bincount(np.asarray(y_train, dtype=int), minlength=2)
    minority_label = int(counts[1] < counts[0])
    truth = np.asarray(y_validation, dtype=int)
    fpr, tpr, thresholds = roc_curve(truth, pred, drop_intermediate=False)
    specificity = 1.0 - fpr
    rows: list[tuple[float, float, float, float]] = []
    for threshold, positive_recall, negative_recall in zip(thresholds, tpr, specificity, strict=True):
        recalls = (float(negative_recall), float(positive_recall))
        minority_recall = recalls[minority_label]
        majority_recall = recalls[1 - minority_label]
        rows.append((float(threshold), minority_recall, majority_recall, float(np.mean(recalls))))
    feasible = [row for row in rows if np.isfinite(row[1]) and row[1] >= target_recall]
    eligible = feasible if feasible else [row for row in rows if np.isfinite(row[1])]
    threshold, minority_recall, majority_recall, balanced = max(
        eligible,
        key=lambda row: (row[2], row[1], row[3], -abs(row[0] - 0.5)),
    )
    return {
        "minority_label": float(minority_label),
        "minority_threshold": threshold,
        "minority_recall": minority_recall,
        "majority_recall": majority_recall,
        "balanced_recall": balanced,
        "minority_constraint_feasible": float(bool(feasible)),
        "minority_recall_target": float(target_recall),
    }
    ...
    for outer_fold, (outer_train, outer_test) in enumerate(outer_splits, start=1):
        ...
        for spec in specs:
            ...
            for inner_fold, (tr_local, va_local) in enumerate(inner_local, start=1):
                ...
                inner_perf = performance(y[va], pred, task_type)
                operating = (
                    minority_operating_point(y[tr], y[va], pred)
                    if task_type == "classification"
                    else {
                        "minority_label": np.nan,
                        "minority_threshold": np.nan,
                        "minority_recall": np.nan,
                        "majority_recall": np.nan,
                        "balanced_recall": np.nan,
                        "minority_constraint_feasible": np.nan,
                        "minority_recall_target": np.nan,
                    }
                )
                ...
                inner_rows.append(
                    {
                        ...
                        "family": spec["family"],
                        "inner_utility": score,
                        "inner_roc_auc": inner_perf["roc_auc"],
                        "inner_pr_auc": inner_perf["pr_auc"],
                        "inner_brier": inner_perf["brier"],
                        **operating,
