...
def main():
    ...
    for task, label, color, marker in [
        ("classification", "Classification, τ=0.010 ROC-AUC", "#0072B2", "o"),
        ("regression", "Regression, τ=0.050 RMSE", "#D55E00", "s"),
    ]:
        series = working[working.task_type == task].groupby("pool_size").crossfit_tau_success.mean()
        ax_b.plot(KS, 100 * series.reindex(KS), marker=marker, linewidth=1.8, color=color, label=label)
    ax_b.set_xticks(KS)
    ax_b.set_ylim(0, 100)
    ax_b.set_xlabel("Candidate count K")
    ax_b.set_ylabel("Cross-fitted τ-success (%)")
    ...
    for task, label, color, marker in [
        ("classification", "Classification, τ=0.010", "#0072B2", "o"),
        ("regression", "Regression, τ=0.050", "#D55E00", "s"),
    ]:
