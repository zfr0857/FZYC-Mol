...
SVG = PACKAGE / "main_figures" / "Figure5.svg"
PNG = PACKAGE / "main_figures" / "Figure5_600dpi.png"
...
def replace(path: Path) -> None:
    ...
    with ZipFile(path) as archive:
        ...
        blips = document.xpath(
            "//*[local-name()='docPr' and @name='Picture 5']"
    ...
    if not targets:
        raise RuntimeError(f"Could not locate Picture 5 in {path.name}")
    replacements = {
    ...
    if len(replacements) != len(targets):
        raise RuntimeError(f"Unsupported Figure 5 media type in {path.name}")
    ...
    try:
        ...
        if replaced != set(replacements):
            raise RuntimeError(f"Incomplete Figure 5 replacement in {path.name}")
    ...
    with ZipFile(path) as archive:
        for member, expected in replacements.items():
            if hashlib.sha256(archive.read(member)).digest() != hashlib.sha256(expected).digest():
                raise RuntimeError(f"Embedded Figure 5 hash mismatch in {path.name}")
