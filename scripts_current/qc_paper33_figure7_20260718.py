...
def main() -> None:
    ...
    text_by_value = {"".join(node.itertext()): node for node in text_nodes}
    headers = [
        ("A", "Opportunity and realised gain"),
        ("B", "Composition-by-K effects"),
        ("C", "Ranking fidelity"),
        ("D", "Downstream budget-benefit"),
    ]
    header_alignment = all(
        abs(float(text_by_value[letter].get("y")) - float(text_by_value[title].get("y"))) < 1e-6
        and abs(
            float(text_by_value[title].get("x"))
            - float(text_by_value[letter].get("x"))
            - 18.0
        ) < 1e-6
    ...
    checks = {
        ...
        "panel_alignment": {"value": "A-D letters share each title baseline and use an 18-pt fixed offset; two-column four-panel grid retained", "status": "pass" if header_alignment else "fail"},
