...
ORIGINAL = ROOT / "output" / "小论文-3.docx"
REVISED = ROOT / "output" / "小论文-4.docx"
OUTPUT = ROOT / "output" / "小论文-4_修订痕迹.docx"
...
def track_inserted_tables(original_root: etree._Element, revised_root: etree._Element, tracker: Tracker) -> None:
    def signature(table: etree._Element) -> str:
        return "|".join(table.xpath(".//w:t/text()", namespaces=NS))

    original_signatures = {signature(table) for table in original_root.xpath(".//w:tbl", namespaces=NS)}
    for table in revised_root.xpath(".//w:tbl", namespaces=NS):
        if signature(table) in original_signatures:
            continue
        for row in table.findall("w:tr", NS):
            row_properties = row.find("w:trPr", NS)
            if row_properties is None:
                row_properties = etree.Element(Q("trPr"))
                row.insert(0, row_properties)
            row_properties.append(tracker.change("ins"))
...
def build() -> Path:
    ...
    track_inserted_tables(original_xml, revised_xml, tracker)
