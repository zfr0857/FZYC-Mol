...
Package version and release: `paper-release-2026-08-r12.9`.
...
container can be built with `docker build -t fzyc-mol:paper-release-2026-08-r12.9 .`.
...
`paper-release-2026-08-r12.9`; its immutable analysis commit is recorded in the GitHub release
...
> paper-release-2026-08-r12.9. GitHub. 2026.
...
Release `paper-release-2026-08-r12.6` is a format-only successor to the immutable
`paper-release-2026-08-r12.5` tag. It is a metadata and submission-format
...
The R12.6 refinement reduces the Figure 6 vertical inter-row spacing from 0.70
to 0.60 while preserving non-overlapping colour strips, headings and axes.

## R12.9 formula and submission synchronization

R12.9 converts the nine main-text estimands and Supplementary Equations
(1)–(22b) to editable Office Math, corrects the set-membership operator in
Equation (1), and synchronizes the equation-to-code inventory. The release also
adds the journal-ready manuscript, supplementary methods, supplementary tables,
supplementary figures, and formula/table audit under `submission_documents/`.
No outer score, prediction, split, table value, figure value, or retained
negative result was changed.
